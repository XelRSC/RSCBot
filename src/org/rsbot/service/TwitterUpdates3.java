...
public class TwitterUpdates3 {

	private TwitterUpdates3() {
	}

	public static void loadTweets(final int count) {
		final Logger log = Logger.getLogger("@" + Configuration.Twitter3.NAME);
		...
		url.append(Configuration.Twitter3.NAME);
		url.append("&count=");
		url.append(count * 25);
		...
		try {
			...
			while ((s = reader.readLine()) != null) {
				...
				if (!msg.contains(Configuration.Twitter3.HASHTAG)) {
					continue;
				}
				if (msg.endsWith(Configuration.Twitter3.HASHTAG)) {
					msg = msg.substring(0, msg.length() - Configuration.Twitter3.HASHTAG.length()).trim();
				}
				if (msg.isEmpty()) {
					continue;
				}
